 ...
 */
  ...
  Shopify.addImageSize = function(src,size){
    ...
    if(src === null){ return "\/\/cdn.shopify.com\/s\/files\/1\/0499\/2866\/7298\/t\/3\/assets\/no-image.gif?v=7079148615093828442" }
    size = '_' + size + '.';
      ...
      if(cart.items.length){
        ...
        for (var i = 0; i < cart.items.length; i++) {
          ...
          var row = '<ul><li><div class="thumb"><a href="' + cart.items[i].url +
              ...
              '</a></h5><p class="product-type">'+title+'</p><p>'+propertiesText+'</p><a href="#" data-id="'+cart.items[i].id+'" class="remove_item_button">Entfernen</a></li>'+
              '<li><p class="mobile-label-price">Preis</p>'+itemPriceContent + ' ' + item_discounts_html + setUnitPrice(cart.items[i]) +'</li>' +
              '<li><p class="mobile-label-quantity">Menge</p><a class="minus_btn"></a><input type="text" name="updates[]" class="txtbox number_val_input" min="0" id="updates_'+cart.items[i].id+'" data-id="'+cart.items[i].id+'" data-line="'+(i+1)+'" value="'+cart.items[i].quantity+'" /><a class="plus_btn"></a>'+
              '<div class="ajaxcart__errors text-center ajaxcart__item__'+cart.items[i].id+'__errors">Alle verfügbaren Lagerbestände befinden sich im Warenkorb</div></li>'+
              '<li><p class="mobile-label-total">Gesamt</p>'+linePriceContent +
        ...
        if(checkoutRow.length){ /* we have existing items in the quick cart */
          ...
          var subtotalRowHtml = '<ul><li>'+discounts_html+'<span class="total_val">Zwischensumme:</span><span class="money total-price">'+totalPriceConverted+'</span></li></ul>';
          ...
          checkoutRow = '<div class="checkout_row clearfix">' + terms_checkbox + '<button type="submit" name="checkout" value="Check Out" class="btn_c">Zur Kasse gehen</button> <a href="javascript:void(0)" class="info_btn"><span class="fa fa-chevron-left"></span>Mit dem Einkaufen fortfahren</a></div>';
          subtotalRow = '<div class="total_row clearfix"><ul><li>'+discounts_html+'<span class="total_val">Zwischensumme:</span><span class="money total-price">'+totalPriceConverted+'</span></li></ul></div>';
            ...
            'novalidate':'novalidate'
              ...
              if(false){
                var notesRow='<div class="clearfix order_notes">'+
                    '<label for="cartSpecialInstructions" class="sr-only">Bestellhinweise:</label>'+
                    '<textarea name="note" id="cartSpecialInstructionsFromPopup" class="note_text" placeholder="Bestellhinweise">'+ cartNote +'</textarea></div>';
              }
          else{
            var notesRow='';
          }
        ...
        t.html('<div class="menu_title clearfix"><h4>Einkaufswagen</h4> </div><div class="cart_row"><div class="empty-cart">Ihr Warenkorb ist derzeit leer.</div><p><a href="javascript:void(0)" class="btn_c btn_close">Mit dem Einkaufen fortfahren</a></p></div>');
        c.html('<div class="empty-cart-message"><p>Ihr Warenkorb ist derzeit leer.</p><p><a href="/collections/all" class="btn_c">Mit dem Einkaufen fortfahren</a></p></div');
       }
      }
